...
import spacy
import argparse
from codon2box import cds2box,box2cds
...
class Bert_CRF(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attn_masks, labels=None):  # dont confuse this with _forward_alg above.
        ...
        if labels is not None:
            ...
            prediction = self.crf.viterbi_decode(emission, mask=attn_masks)
            return prediction
...
def generate_training_data(config, bert_tokenizer="bert-base", do_lower_case=True):
    ...
    eval_iter = data.DataLoader(dataset=dev_dataset,
                                ...
                                num_workers=0,
                                collate_fn=pad)
    print(len(train_iter),len(eval_iter))
    return train_iter, eval_iter, tag2idx

def generate_test_data(config, tag2idx, input_file,bert_tokenizer="bert-base", do_lower_case=True):
...
def train(train_iter, eval_iter, tag2idx, config, bert_model="bert-base-uncased"):
    ...
    for epoch in (train_iterator):
        ...
        val_loss = 0.0
        model.eval()
        writer = open(config.apr_dir + 'prediction_'+str(epoch)+'.txt', 'w')
        ...
        print("Validation Loss: %f for epoch %d" %(val_loss/len(eval_iter), epoch))        
        validation_loss.append(val_loss/len(eval_iter))
        writer.flush()
        print('Epoch: ', epoch)
        command = "python ./conlleval.py < " + config.apr_dir + "prediction_"+str(epoch)+".txt"
...
'''
def test(test_iter, model, unique_labels, out_file):
    model.eval()
    writer = open(out_file+'prediction.txt', 'w')
    for i, batch in enumerate(test_iter):
        ...
        for i in range(len(sorted_idx)):
            ...
            writer.write('\n')
    writer.flush()
    box2cds(out_file+'prediction.txt',out_file+'opt_cds.txt')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_mode', type=str, default='test', help='Mode of the model (train or test).')
    parser.add_argument('--input_file', type=str, default='./data/', help='Path to the input TXT data file.')
    parser.add_argument('--out_file', type=str, default='./result/', help='Save the output file.')
    ...
    if args.model_mode == "train":
        ...
        cds2box(args.input_file+'test.txt',args.input_file+"cdbox.txt")
        model, bert_tokenizer, unique_labels, tag2idx = load_model(config=config, do_lower_case=True)
        test_iter = generate_test_data(config, tag2idx,args.input_file+"cdbox.txt",bert_tokenizer=config.bert_model, do_lower_case=True)
        print('Testing...')
        print('Convert the CDS sequence to an amino acid sequence, and use BIO annotation to label the codon box.')
        print('Number of test seq: ', len(test_iter))
        test(test_iter, model, unique_labels, args.out_file)
        print('The output file is saved in ', args.out_file+'opt_cds.txt')
        print('Done!')
