...
def train(train_iter, eval_iter, tag2idx, config, bert_model="bert-base-uncased"):
    ...
    for epoch in (train_iterator):
        ...
        for i, batch in enumerate(eval_iter):
            ...
            #print(len(token_ids),len(attn_mask),len(org_tok_map),len(labels),len(original_token),len(sorted_idx))
            #attn_mask.dt
            inputs = {'input_ids': token_ids.to(device),
                      'attn_masks' : attn_mask.to(device)
                     }  
        ...
        writer.flush()
        print('Epoch: ', epoch)
        command = "python D:/bert_crf-master/bert-crf4NER/conlleval.py < " + config.apr_dir + "prediction_"+str(epoch)+".txt"
...
def test(config, test_iter, model, unique_labels, test_output):
    ...
    for i, batch in enumerate(test_iter):
        ...
        for i in range(len(sorted_idx)):
            ...
            writer.write('\n')
    writer.flush()
    command = "python D:/bert_crf-master/bert-crf4NER/conlleval.py < " + config.apr_dir + test_output
    process = subprocess.Popen(command,stdout=subprocess.PIPE, shell=True)
    result = process.communicate()[0].decode("utf-8")
    print(result)
